...
export default function EpochHistoryPage() {
    ...
    useEffect(() => {
        if (selectedEpoch) {
            fetchSnapshots(selectedEpoch.epochId);
        }
    ...
    const fetchEpochs = async () => {
        setIsLoading(true);
        try {
            const q = query(collection(db, "epochs"), orderBy("epochId", "desc"), limit(20));
            const snapshot = await getDocs(q);
            const epochData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setEpochs(epochData);
                                ...
                                <div className="flex justify-between items-center relative z-10">
                                    <div>
                                        <div className="text-[9px] font-black uppercase tracking-tighter opacity-70">
                                            {new Date(epoch.startTime.toMillis()).toLocaleDateString()}
                                        </div>
